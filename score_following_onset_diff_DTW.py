...
#Rc = 74
DIRECTORY = '/Users/rtchen/Downloads/sing_dataset/0021/'
...
# NEWFILE = os.path.join(DIRECTORY,'generated_{}filter5.mid'.format(FILTER))
# CONF_FILE = os.path.join(DIRECTORY,'{}filter_confidence5.txt'.format(FILTER))
...
def compute_f_I_given_D(fsource,f_I_J_given_D,cur_pos,scoreLen):
    ...
    right = min(scoreLen,cur_pos+200)
...
def move_fsource(diff_pos,fsource,cur_pos):
    fsource_new = np.zeros(len(fsource))
    for i in range(max(0,cur_pos-50),min(cur_pos+101,len(fsource))):
    #for i in range(0,len(fsource)):
        if i-diff_pos>=0 and i-diff_pos<len(fsource):
            fsource_new[i] = fsource[i-diff_pos]
    return fsource_new
...
def score_follow(audio_file,midi_file ,feature,mask):
    ...
    while wf.tell() < wf.getnframes():  
        ...
        pos_diff = 0
        if adjust_cnt < CNT_CONSTANT:
            adjust_cnt = adjust_cnt+1
        if adjust_cnt ==CNT_CONSTANT:
            if len(raw_pitches)>=31 and cur_pos>=99:
                A_array = np.asmatrix(raw_pitches[-31:])
                B_array = np.asmatrix(score_midi[cur_pos-99:cur_pos+1])
                len_A = len(A_array)
                len_B = len(B_array)
                D, wp = librosa.sequence.dtw(A_array, B_array)
                # D, wp = librosa.dtw(A_array,B_array)
                alignCost= D[len_A-1,len_B-1]
                cost1 = alignCost
                pos_diff = 0
                time_gap = [-25,-15,-10,10,11,12,13,14,15,18,20,24,25,26,30]
                for t in time_gap:
                    if len(raw_pitches)>=31 and cur_pos>=99+t and cur_pos+1-t<=len(score_midi):
                        A_array = np.asmatrix(raw_pitches[-31:])
                        B_array = np.asmatrix(score_midi[cur_pos-(99+t):cur_pos+1-t])
                        len_A = len(A_array)
                        len_B = len(B_array)
                        D, wp = librosa.sequence.dtw(A_array, B_array)
                        # D, wp = librosa.dtw(A_array,B_array)
                        alignCost2 = D[len_A-1,len_B-1]
                        if alignCost2> alignCost:
                            alignCost = alignCost2
                            pos_diff = -t
            if pos_diff!=0:
                adjust_cnt = 0
        ...
        if pos_diff and alignCost-cost1>30:
            print 'here-----------------------%d'%pos_diff
            print 'diff %f'%(alignCost-cost1)
            fsource = move_fsource(pos_diff,fsource,cur_pos)
